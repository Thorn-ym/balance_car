...
public class MainActivity extends Activity {
    ...
    private static final long CONTROL_PERIOD_MS = 120L;
    private static final float MAX_SPEED = 0.8f;
    private static final float MAX_TURN = 0.8f;
    ...
    private void sendControlTargets(boolean forceChanged) {
        ...
        if (forceChanged || Math.abs(currentSpeed) > 0.001f || Float.isNaN(lastSentSpeed)) {
            if (forceChanged || Math.abs(currentSpeed - lastSentSpeed) > 0.01f || Math.abs(currentSpeed) > 0.001f) {
                sendCommand(String.format(Locale.US, "SPD %.2f", currentSpeed), false);
                lastSentSpeed = currentSpeed;
            }
        }
        if (forceChanged || Math.abs(currentTurn) > 0.001f || Float.isNaN(lastSentTurn)) {
            if (forceChanged || Math.abs(currentTurn - lastSentTurn) > 0.01f || Math.abs(currentTurn) > 0.001f) {
                sendCommand(String.format(Locale.US, "TURN %.2f", currentTurn), false);
